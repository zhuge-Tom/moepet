...
# The desktop pet has one fixed, authorized reference clip per process. Build
# its semantic/spec cache before accepting requests, then release HuBERT; later
# requests reuse the cache and keep only the models needed for synthesis.
_moepet_reference = os.environ.get("MOEPET_TTS_REFERENCE", "").strip()
if _moepet_reference:
    tts_pipeline.set_ref_audio(_moepet_reference)
    _moepet_prompt = os.environ.get("MOEPET_TTS_PROMPT", "").strip()
    if _moepet_prompt:
        # Pay Japanese frontend and decoder cold costs while Moepet is starting,
        # not after the user receives the first reply.
        for _ in tts_pipeline.run({
            "text": "おはようございます。", "text_lang": "all_ja",
            "ref_audio_path": _moepet_reference,
            "prompt_text": _moepet_prompt, "prompt_lang": "all_ja",
            "text_split_method": "cut0", "batch_size": 1,
            "speed_factor": 1.0, "streaming_mode": False,
            "parallel_infer": False,
        }):
            pass
    tts_pipeline.cnhuhbert_model = None
    gc.collect()
